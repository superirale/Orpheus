...
TEST_CASE("VoicePool AllocateVoice", "[VoicePool]") {
  ...
  DistanceSettings ds;
  ds.maxDistance = 100.0f;
  Voice *v = pool.AllocateVoice("test_event", 128, {0, 0, 0}, ds);
  REQUIRE(v != nullptr);
...
TEST_CASE("VoicePool voice IDs are unique", "[VoicePool]") {
  ...
  // reallocation
  DistanceSettings ds;
  ds.maxDistance = 50.0f;
  VoiceID id1 = pool.AllocateVoice("event1", 128, {0, 0, 0}, ds)->id;
  VoiceID id2 = pool.AllocateVoice("event2", 128, {0, 0, 0}, ds)->id;
  VoiceID id3 = pool.AllocateVoice("event3", 128, {0, 0, 0}, ds)->id;
...
TEST_CASE("VoicePool MakeReal changes state", "[VoicePool]") {
  ...
  DistanceSettings ds;
  ds.maxDistance = 100.0f;
  Voice *v = pool.AllocateVoice("test", 128, {0, 0, 0}, ds);
  REQUIRE(v->state == VoiceState::Virtual);
...
TEST_CASE("VoicePool GetActiveVoiceCount", "[VoicePool]") {
  ...
  DistanceSettings ds;
  ds.maxDistance = 50.0f;
  (void)pool.AllocateVoice("e1", 128, {0, 0, 0}, ds);
  (void)pool.AllocateVoice("e2", 128, {0, 0, 0}, ds);
